...
| `V2.5` | `identity.admin_audit_event` | 7 (merged pending) |
| `V2.6` | `identity.self_service_request` | 8 (merged pending) |
...
- 2026-07-15: Chunk 8 (identity-local export & deletion) implemented on `feat/002-identity-chunk8-export-delete`, branched from `main` after chunk 7 merged (`7161919`) — the final chunk. `POST /api/me/export` and `DELETE /api/me`, both requiring `Idempotency-Key`. Reuses the existing `Account.close`/`PublicProfile.anonymize` domain primitives (no rewrite). New `V2.6` `identity.self_service_request` (unique `(account_id, idempotency_key)`) records DSRs and gates key reuse; a `SelfServiceRequestRegistrar` returns FIRST/REPLAY and throws `IdempotencyKeyConflictException` on same-key/different-type, translating the concurrent-insert race like chunks 4–5. Key design realization: `close`/`anonymize` are idempotent, so the mutation always runs and needs no transaction shared with the idempotency record — keeping application services framework-free. Export includes the user's own email (its purpose); deletion scrubs email, tombstones the pseudonym, drops the avatar, and — via chunk 5 — blocks re-provisioning of the deleted subject. `applyDeletion` persists account closure + profile anonymization atomically in `JpaIdentityPersistenceAdapter`. Tests: registrar/export/deletion unit tests plus `SelfServiceEndpointIntegrationTest` proving export, deletion (DB-verified CLOSED + null email + `del-` tombstone), **no-resurrection (post-delete request → 401)**, key-reuse conflict (409), missing key (400), and reachable export replay. Note: a *repeated DELETE* through the endpoint is not reachable — a deleted account is 401'd at auth — so DELETE idempotency is asserted at the service layer, not the endpoint. `./scripts/check.sh` passes. This completes plan `002` (8/8), pending chunk 8's independent review and merge.
- 2026-07-15: Chunk 7 (admin RBAC proof-of-concept + audit) implemented on `feat/002-identity-chunk7-admin-audit`, branched from `main` after chunk 6 merged (`1851e62`). `GET /api/admin/accounts/{id}` gated by `/api/admin/** hasRole('ADMIN')` in the app `SecurityConfiguration` (role resolved server-side per ADR-0005). New `V2.5` migration adds the append-only `identity.admin_audit_event`; **`target_account_id` is intentionally not a foreign key** so a lookup of a non-existent account is still audited (NOT_FOUND) — a regression the integration test now guards after the first cut's FK rejected exactly that insert. New domain `AdminAuditEvent`/`AdminAuditAction`/`AdminAuditOutcome`; `AdminAuditEventRepository` port + JPA adapter; `AdminAccountService` (replaces the pure `AdminAccountLookupService`) looks up + audits every call and is fail-closed (an audit-write failure propagates, so no account data is returned unaudited). `AdminAccountView` DTO omits `authSubjectHash` and email (discharges the review carry-forward); a shared `WebAuthentication.accountId` helper now serves both controllers. First-ADMIN promotion is a manual SQL `UPDATE` by design. Tests: `AdminAccountServiceTest` (found/not-found/audit-failure) and `AdminAccountEndpointIntegrationTest` (401 anon, 403 USER, 200 ADMIN with no `authSubjectHash` in the body + audit row, 404 unknown + NOT_FOUND audit row). `./scripts/check.sh` passes. Deferred: export/delete + idempotency (chunk 8, `V2.6`).
...
All eight chunks are implemented. Chunks 1–7 are merged to `main`; chunk 8
(`feat/002-identity-chunk8-export-delete`) is code-complete and awaiting its independent review and
merge. Every acceptance criterion is met and exercised end-to-end against Testcontainers Postgres:
vendor-agnostic JWT auth with DB-resolved roles (never a JWT claim), atomic first-login
provisioning, `/api/me` read/update with the full negative-path matrix, admin RBAC with an
append-only audit trail, and identity-local export/deletion with idempotency and the no-resurrection
guarantee. `identity.api` remains an intentional empty stub (no consumer yet). Deferred beyond this
plan: MFA/step-up and a real IdP vendor (ADR-0005), audit-log retention/read-restriction policy,
rate limiting, and cross-module DSR orchestration (no other module holds data yet).
