...
Use focused read-only subagents where they add independent value. Keep scope bounded. Add tests alongside the change. Update relevant docs/contracts. Run L0 narrow checks while editing and L1 scoped checks before handoff. Do not repeatedly run `./scripts/check.sh`; the lead/CI runs the L2 gate once per merge candidate.
