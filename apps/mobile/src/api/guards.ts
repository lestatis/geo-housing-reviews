 ...
 * A response that is 200 but does not match the generated type is `malformed` (plan 023, "Error
 * model"), and the only way to know that is to check the fields the caller actually reads. These
 * helpers are the smallest set needed to write those checks; a schema-validation dependency was
 * deliberately not added for it.
 */
