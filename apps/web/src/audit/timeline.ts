 ...
 *
 * <p>The endpoint defaults to the last seven days. A screen that quietly applied that default would
 * let somebody conclude nothing happened when they were looking at the wrong week — which, for an
 * audit log, is the failure that matters.
 */
export function describeWindow(
  ...
  const end = until?.trim() || asDate(now);
  const start =
    since?.trim() || asDate(new Date(now.getTime() - DEFAULT_DAYS * 24 * 60 * 60 * 1000));
